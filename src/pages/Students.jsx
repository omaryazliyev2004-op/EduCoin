...
export default function Students() {
    ...
    const [archivedStudentIds, setArchivedStudentIds] = useState(() => {
        try {
            const saved = localStorage.getItem("archivedStudentIds")
            if (!saved) return []
    ...
    useEffect(() => {
        if (Array.isArray(archivedStudentIds)) {
            localStorage.setItem("archivedStudentIds", JSON.stringify(archivedStudentIds))
        }
    }, [archivedStudentIds])

    // API orqali o'quvchilarni yuklash
    const fetchStudents = async () => {
        ...
        try {
            ...
            const mappedStudents = backendData.map((s) => ({
                id: s.id,
                name: s.full_name,
                avatar: s.photo 
                ...
                birth: s.birth_date ? s.birth_date.substring(0, 10) : "2005-01-01",
            ...
            const fetchedIds = mappedStudents.map(s => s.id)
            setArchivedStudentIds(prev => prev.filter(id => fetchedIds.includes(id)))
        } catch (err) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => fetchStudents(), 0)
        return () => clearTimeout(timer)
    }, [])

    const baseStudents = isArchiveView 
        ? students.filter(s => archivedStudentIds.includes(s.id)) 
        : students.filter(s => !archivedStudentIds.includes(s.id))

    const filteredStudents = useMemo(() => {
        return baseStudents.filter(s => s.name.toLowerCase().includes(searchQuery.toLowerCase()))
    ...
    const isSomeSelected = selectedIds.length > 0 && selectedIds.length < filteredStudents.length
    ...
    const handleBulkDelete = () => {
        setModalConfig({
            ...
            message: isArchiveView 
                ? `Siz haqiqatan ham tanlangan ${selectedIds.length} ta talabani butunlay o'chirib tashlamoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi!`
                : `Siz haqiqatan ham tanlangan ${selectedIds.length} ta talabani arxivga ko'chirmoqchimisiz?`,
            type: "danger",
            ...
            onConfirm: async () => {
                ...
                try {
                    if (isArchiveView) {
                        ...
                        setArchivedStudentIds(prev => prev.filter(id => !selectedIds.includes(id)))
                        setStudents(prev => prev.filter(s => !selectedIds.includes(s.id)))
                    } else {
                        // Soft delete (arxivga o'tkazish)
                        setArchivedStudentIds(prev => [...new Set([...prev, ...selectedIds])])
                    }
                    setSelectedIds([])
                } catch (err) {
                    console.error("Guruhli o'chirishda xatolik:", err)
                    setError("Tanlangan talabalarni o'chirishda xatolik yuz berdi.")
                } finally {
                    setLoading(false)
                }
            }
    ...
    const handleClose = () => { setShowDrawer(false); setEditStudent(null); }
    ...
    const handleRestore = (id) => {
        ...
        setModalConfig({
            open: true,
            title: "Talabani arxivdan qaytarish?",
            message: `Siz haqiqatan ham "${student?.name || ''}" talabasini arxivdan qayta tiklamoqchimisiz?`,
            type: "restore",
            confirmText: "Qayta tiklash",
            onConfirm: () => {
                setArchivedStudentIds(prev => prev.filter(item => item !== id))
            }
        })
    }
    ...
    const handleDelete = (id) => {
        ...
        setModalConfig({
            ...
            message: isArchiveView 
                ? `Ushbu talaba ("${student?.name || ''}") arxivdan butunlay o'chib ketadi. Bu amalni qaytarib bo'lmaydi!` 
                : `Ushbu talaba ("${student?.name || ''}") arxivga ko'chiriladi.`,
            ...
            onConfirm: async () => {
                ...
                try {
                    if (isArchiveView) {
                        ...
                        setArchivedStudentIds(prev => [...prev, id])
                    }
                } catch (err) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editStudent) {
                ...
                formData.append("phone", form.phone)
                formData.append("email", form.email || `${form.phone.replace(/[^0-9]/g, "")}@tm-edu.uz`)
                formData.append("password", form.password || "Student123!") 
                formData.append("address", form.address || "Toshkent")
                if (form.birth) {
                    formData.append("birth_date", new Date(form.birth).toISOString())
                }
            ...
            console.error("Talabani saqlashda xato:", err)
            setError(err.response?.data?.message || err.response?.data?.detail || err.message || "Talabani saqlashda xatolik yuz berdi.")
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <div className="-m-6 min-h-full bg-[#f4f6fb] p-8">
            <div className="flex items-start justify-between mb-6">
                <div>
                    <h1 className="text-[27.5px] font-extrabold text-[#1f2d5a] tracking-tight">{isArchiveView ? "Arxiv - Talabalar" : "Talabalar"}</h1>
                    <p className="text-[15px] text-gray-500 mt-1">
                        {isArchiveView ? "Ushbu sahifada o'chirilgan talabalar arxivi keltirilgan." : "Ushbu sahifada siz talabalar ro'yxatini va ularning ma'lumotlarini topasiz."}
                    </p>
                </div>
            ...
            {isArchiveView && (
                <div className="flex items-center gap-3 mb-5 px-4 py-3 rounded-xl bg-amber-50 border border-amber-100 animate-in fade-in slide-in-from-top-1 duration-200">
                    <span className="text-[13.5px] text-amber-700 font-medium">
                        Bu yerda o'chirilgan talabalar arxivi saqlanadi. Ularni qayta tiklash yoki butunlay o'chirish mumkin.
                    </span>
                </div>
            )}
            ...
            {selectedIds.length > 0 && (
                <div className="flex items-center justify-between px-6 py-3.5 mb-5 rounded-2xl bg-red-50/80 border border-red-100 animate-fade-in-up">
                    <span className="text-[15px] font-semibold text-red-600">
                        {selectedIds.length} ta talaba tanlandi
                    </span>
                    <Button
                        variant="contained"
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center gap-4 bg-white rounded-[24px] border border-gray-100 px-6 py-20 text-center">
                    <div>
                        <p className="text-[16.5px] font-bold text-[#1f2d5a]">Talabalar topilmadi.</p>
                        <p className="mt-1 text-[14.5px] text-gray-400">
                            {searchQuery.trim()
                                ? "Qidiruv bo'yicha mos talaba yo'q. Qidiruvni tozalab ro'yxatga qayting."
                                : isArchiveView
                                    ? "Arxivda talaba topilmadi. Asosiy ro'yxatga qaytishingiz mumkin."
            ...
            <div className={`fixed top-0 right-0 h-full w-[440px] bg-white shadow-2xl z-[300] flex flex-col transition-transform duration-300 ease-in-out ${showDrawer ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className="flex items-start justify-between px-6 pt-6 pb-4">
                    <div>
                        <h3 className="text-[18.5px] font-bold text-gray-800">{editStudent ? "Talabani tahrirlash" : "Talaba qo'shish"}</h3>
                        <p className="text-[14px] text-gray-400 mt-0.5">Bu yerda siz talaba ma'lumotlarini kiritishingiz mumkin.</p>
                    </div>
                ...
                <div className="flex-1 px-6 py-4 space-y-5 overflow-y-auto">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[14.5px] font-semibold text-gray-700">Talaba FIO</label>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input type="text" value={form.address} onChange={e => setForm({...form, address: e.target.value})} placeholder="Masalan: Tashkent sh., Chilonzor tumani" className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-[14.5px] focus:border-violet-400 outline-none" />
                    </div>
                    <div className="space-y-1.5">
